...
{
    private static readonly Lazy<Func<T, object?, bool>?> _lazyEqualsObjectFunc =
        new(CreateEqualsObjectFunc, LazyThreadSafetyMode.ExecutionAndPublication);
    ...
    private static Func<T, object?, bool>? CreateEqualsObjectFunc()
    {
        ...
        var dynamicMethod =
            DynamicMethod.New($"{TypeName.For<T>()}_Equals_Object", typeof(bool), typeof(T), typeof(object));
    ...
    public static bool Equals(T? value, object? other)
    {
        ...
        if (func is not null)
        {
            return func.Invoke(value, other);
        }
