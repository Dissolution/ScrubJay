...
#region Constructors
    internal SplitTextEnumerator(text source, char separator, StringComparison comparison = StringComparison.Ordinal)
    {
        Source = source;
    ...
    internal SplitTextEnumerator(text source, text separator, bool treatAsSingleSeparator, StringComparison comparison = StringComparison.Ordinal)
    {
        Source = source;
...
#if NET8_0_OR_GREATER
    internal SplitTextEnumerator(text source, SearchValues<char> searchValues, StringComparison comparison = StringComparison.Ordinal)
    {
        Source = source;
    ...
    public bool MoveNext()
    {
        ...
        switch (_splitMode)
        {
            ...
            {
                separatorIndex = Source[_scanStartIndex..].IndexOf(_separator.AsSpan(), _comparison);
                break;
            }
            case SplitMode.Any:
            {
                separatorIndex = Source[_scanStartIndex..].IndexOf(_separatorBuffer, _comparison);
                break;
            }
            ...
            {
                separatorIndex = Source[_scanStartIndex..].IndexOfAny(_searchValues!);
                break;
            }
#endif
