...
public abstract class EnumMember
{
    ...
    protected EnumMember(Type enumType, FieldInfo memberField)
    {
        ...
        if (Attributes.HasAttribute<EnumMemberAttribute>(out var enumMemberAttribute))
        {
            AddAlias(enumMemberAttribute.Value);
        }
        if (Attributes.HasAttribute<DescriptionAttribute>(out var descriptionAttribute))
        {
            AddAlias(descriptionAttribute.Description);
        }
#if !(NETFRAMEWORK || NETSTANDARD)
            if (Attributes.HasAttribute<DisplayAttribute>(out var displayAttribute))
            {
