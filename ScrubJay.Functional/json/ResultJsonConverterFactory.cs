...
/// <summary>
/// A <see cref="JsonConverterFactory"/> that produces <see cref="ResultJsonConverter{T}"/> or <see cref="ResultJsonConverter{T,E}"/> instances.
/// </summary>
[PublicAPI]
public sealed class ResultJsonConverterFactory : JsonConverterFactory
{
    ...
    public override JsonConverter CreateConverter(Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        else if (genericTypeDef == typeof(Result<,>))
        {
            object inst = Activator.CreateInstance(typeof(ResultJsonConverter<,>)!.MakeGenericType(genericTypes))!;
            if (inst is JsonConverter converter)
                return converter;
