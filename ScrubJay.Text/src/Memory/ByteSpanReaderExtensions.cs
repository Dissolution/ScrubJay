...
#region Text
    ...
    public static string ReadString(
        ...
        Endianness endianness = Endianness.System,
        Encoding? encoding = null)
        => reader.ReadString(checked((int)length), endianness, encoding);
...
#region Time
        ...
        TimeEncodingAffix fix)
    {
        switch (fix)
        {
            ...
            {
                long ticks = ReadI64(reader);
                return new TimeSpan(ticks);
            }
            case TimeEncodingAffix.TimeU32:
            {
                uint seconds = ReadU32(reader);
                return TimeSpan.FromSeconds(seconds);
            }
            case TimeEncodingAffix.TimeU64:
            {
                ulong seconds = ReadU64(reader);
                return TimeSpan.FromSeconds(seconds);
            }
            default:
                throw Ex.UndefinedEnum(fix);
