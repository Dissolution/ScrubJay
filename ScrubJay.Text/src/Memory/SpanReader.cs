...
public ref struct SpanReader<T>
{
    ...
    public void SkipWhile(Func<T, bool> predicate)
    {
        ...
        {
            if (!predicate(span[pos]))
                break;
        }
        _position = pos;
    }

    public void SkipWhileMatching(scoped ReadOnlySpan<T> match)
    {
        int matchLen = match.Length;
        var span = _span;
        int pos = _position;
        int end = span.Length - matchLen; // inclusive
        while (pos <= end)
        {
            if (!Equate.Sequences(span.Slice(pos, matchLen), match))
            {
                break;
            }
            pos += matchLen;
