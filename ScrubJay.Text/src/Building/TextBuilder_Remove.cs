...
public ref partial struct TextBuilder
{
    public Result<char> TryGetAndRemoveAt(Index index)
    {
        ...
        char removed = _charArray[offset];
        var right = _charArray.AsSpan(offset + 1);
        TextHelper.Notsafe.CopyBlock(right, _charArray.AsSpan(offset), right.Length);
        _position--;
        return Ok(removed);
    }
    ...
    public Result<char[]> TryGetAndRemoveAt(Range range)
    {
        ...
        char[] removed = _charArray.AsSpan(offset, len).ToArray();
        var right = _charArray.AsSpan(offset + len);
        TextHelper.Notsafe.CopyBlock(right, _charArray.AsSpan(offset), right.Length);
