...
public ref partial struct TextBuilder
{
    ...
    public Span<char> Allocate(int length)
    {
        ...
        Span<char> slice = _charArray.AsSpan(pos, length);
    ...
    public Span<char> AllocateAt(Index index, int length)
    {
        ...
        _charArray.SelfCopy(Range.OffsetLength(offset, length), (offset+length)..);

        // the hole we created
        Span<char> slice = _charArray.AsSpan(offset, length);
